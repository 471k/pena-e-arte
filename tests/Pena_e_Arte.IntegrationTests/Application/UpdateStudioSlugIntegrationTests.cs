...
public class UpdateStudioSlugIntegrationTests(DatabaseFixture fixture)
{
    ...
    public async Task UpdateStudioSlug_ValidSlug_UpdatesSlugInDatabase()
    {
        ...
        StudioResponse registered = await RunRegisterHandler(new("Slug Studio", originalSlug, "Lisboa", 38.7, -9.1, "owner@slugstudio.com", UniqueTestNipt(), "123 Test St"));
    ...
    public async Task UpdateStudioSlug_SlugLockedAfterFirstChange_PreventsSecondChange()
    {
        ...
        StudioResponse registered = await RunRegisterHandler(new("Lock Studio", originalSlug, "Porto", 41.1, -8.6, "owner@lockstudio.com", UniqueTestNipt(), "123 Test St"));
    ...
    public async Task UpdateStudioSlug_DuplicateSlug_ThrowsBusinessRuleViolationException()
    {
        ...
        await RunRegisterHandler(new("Taken Studio", takenSlug, "Braga", 41.5, -8.4, "owner@taken.com", UniqueTestNipt(), "123 Test St"));

        string ownSlug = UniqueSlug();
        StudioResponse mine = await RunRegisterHandler(new("My Studio", ownSlug, "Faro", 37.0, -7.9, "owner@mine.com", UniqueTestNipt(), "123 Test St"));
    ...
    public async Task UpdateStudioSlug_SecondChange_ThrowsBusinessRuleViolationException()
    {
        ...
        StudioResponse studio = await RunRegisterHandler(new("Two-Change", original, "Evora", 38.5, -7.9, "owner@twochange.com", UniqueTestNipt(), "123 Test St"));
