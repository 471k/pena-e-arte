...
public class GetStudioReviewsHandlerTests
{
    ...
    public async Task Projection_includes_owner_response_fields_when_present()
    {
        ...
        Review review = Review.ForStudio(studio.Id, Guid.NewGuid(), Guid.NewGuid(), "Ana Silva", 5, "Wonderful experience!");
    ...
    {
        Studio studio = await SeedStudio();
        _db.Reviews.Add(Review.ForStudio(studio.Id, Guid.NewGuid(), Guid.NewGuid(), "Ana Silva", 5, "Great work!"));
        await _db.SaveChangesAsync();
