...
| 40 | Client Account Deletion Hardening | `Client.ArchivedAt`; fixed `ClientDataErasure` fan-out | `ClientAccountExtensions.FindAllClientRecordsForUserAsync` (approved usage #29) | Per-tenant (archive/restore), cross-tenant (erasure/export/cancel fan-out, same person) |
...
Plan and its prices are split into two tables — see the "Plan/PlanPrice split"
Decisions Log row for why (an earlier single-row-per-interval Plan schema couldn't
represent "no Yearly price yet" without a fake/duplicate row).

```
Plan (Admin-owned, not tenant-scoped)
  PlanId, Name, AllowBrandingRemoval, AllowApiAccess, PrioritySupport,
  AllowMarketingCampaigns, MaxArtists, MaxAppointmentsPerMonth,
  MaxNotificationsPerMonth, MaxStorageGb, MaxLocations
  YearlyDiscountPercent (admin-input only — feeds the suggested-yearly-price helper,
  never read by an owner-facing surface; see "Yearly Discount" below)

PlanPrice (one row per interval a Plan actually offers)
  PlanPriceId, PlanId, Interval (Monthly | Yearly), Price, StripePriceId (nullable
  until linked), IsActive (false = retired — kept for history, never purchasable)

Subscription (links Studio → Plan)
  SubscriptionId, StudioId, PlanId, BillingInterval (Monthly | Yearly)
...
Every paid core tier offers Yearly at Monthly × 10 (2 months free). The owner-facing
saving is computed from the two real `PlanPrice` rows — never typed or read from
`Plan.YearlyDiscountPercent`, which only feeds the admin plan editor's
suggested-yearly-price helper. See "Four-tier catalogue + yearly on every paid tier"
...
| 53 | `GetNotificationsQuery` (admin branch) | Cross-tenant `NotificationLog` read — the admin's own `tenant_id` claim would otherwise silently scope this to one arbitrary studio instead of returning every `RecipientType.Admin` row platform-wide | AdminOnly |
...
| Two-stage retention purge + R2 delete + audited erasure (EPIC-0001 PENA-104) — 2026-07-31 | `RetentionPurgeJob` (registered in `Program.cs` via `IRecurringJobManager.AddOrUpdate`, Cron.Daily(4) — NOT `IJobScheduler`, which has no cron concept) soft-deletes consent forms past `App:RetentionDays:ConsentForms`, then hard-purges rows past the grace window, deleting the R2 object first via the NEW `IR2Service.DeleteAsync`. Retention numbers are configurable PLACEHOLDERS (open question §3.6). `RequestDataErasureCommand` (owner/support endpoint, `OwnerOnly`) soft-deletes a client's consent forms + profile immediately, audited with a DISTINCT action (`Client.DataErasureRequested`) from the automatic purge (which writes no audit row). Client-facing self-service erasure UI shipped in a later, undated session (`RequestMyDataErasureCommand`, `DeleteAccountSection.tsx`) — this row was not updated at the time; corrected 2026-09-18, see that date's Decisions Log entry for the cross-studio fan-out bug found in it. | GDPR Art. 5(1)(e)/Art. 17, NIST SP 800-53 SI-12; two-stage soft-delete/hard-purge mirrors S3 lifecycle. Verified: dotnet build/format/test green; new endpoint has `RequireAuthorization`. |
...
| CSV export, booking-style field, installable PWA — P1 Group 1 (2026-09-09) | Three owner-only typed CSV export endpoints (clients/appointments/revenue-ledger), each a MediatR query + FluentValidation validator + hand-rolled `CsvUtils` (no CsvHelper — no-new-NuGet constraint), reached from the frontend via a new `downloadAuthenticatedFile` helper (takes token/tenantId as params rather than reading the Redux store directly, per this file's own "no direct `store.getState()` outside Redux middleware" convention — a deviation from the source overnight prompt's draft, which read the store directly). `BookingIntake.Style` (string?, `AddStyleToBookingIntake` migration) threaded through `CreateAppointmentRequest`/`AppointmentResponse`/both booking validators, validated against `TattooStyle.All`; frontend gained `shared/constants/tattooStyles.ts` as the single shared source, adopted by `TattooIntakeFields.tsx` (new Style `Select`, shared by both booking forms) and — beyond the prompt's own scope, closing a doc-comment-flagged duplication risk found while doing the PortfolioFeed.tsx refactor the prompt did ask for — by `ArtistPortfolioPage.tsx` as well, which had an undocumented third copy of the same list. Installable PWA: hand-rolled `manifest.json`/`sw.js` (cache-first app shell, always-network `/api/`/`/hubs/`), icons generated locally via ImageMagick (SVG→PNG rasterization of the real brand mark failed silently on this machine's ImageMagick build — ended up hand-drawing a solid violet-600 background with a centered "T" mark instead, the source prompt's own named fallback path). Full deviation/trace log: `docs/claude/overnight-prompt-p1-group1-2026-09-09.md`'s own addendum note and the P1 backlog spec's 2026-09-09 addendum. | Current vertical-booking-SaaS standard (CLAUDE.md rule 6) — CSV export and PWA installability are baseline expectations at this product tier (Fresha/Vagaro/Boulevard/GlossGenius-class); style-upfront booking closes a real UX gap where style was previously buried in freeform text. Item 6 (Saved Payment Method) deliberately deferred — still blocked on the POK/payment-provider decision (`IPaymentProvider`/`NullPaymentProvider`, ADR-0001) — see the P1 backlog spec's Context section. |
...
| Studio street address + live forward-geocoding (2026-09-16) | Three new nullable `string` columns on `Studio` — `AddressLine1` (300), `AddressLine2` (150), `PostalCode` (20) — nullable for backfill at the DB layer, `AddressLine1` required at the app layer only for new registrations (`RegisterStudioValidator`), same "required-at-the-app-layer, nullable-at-the-DB-layer" shape `Nipt` already established; `AddressLine2`/`PostalCode` stay optional forever, same as `PhoneNumber`/`InstagramHandle`. New shared hook `useAddressGeocode` (debounced 700ms, 5-char minimum, stale-response-guarded via a `latestAddressRef`) forward-geocodes the typed address against Nominatim and feeds the result through `LocationPicker`'s existing externally-driven `setValue(lat/lng/city)` path — `location-picker.tsx` itself has a zero-line diff, no new prop was added to its public interface. One-directional by design: the pin is the source of truth once dropped/dragged, the address text is never rewritten from a reverse-geocoded pin position. Wired identically on `RegisterStudioPage` (required field, Step 1) and `StudioProfilePage` (optional on update, with a dismissible per-session backfill banner mirroring the existing NIPT-banner mechanism). On the public side, `PublicStudioResponse` gains all three fields and `StudioPortfolioPage` renders the full address (falling back to city-only pre-backfill) — the existing coordinate-based "Get Directions" link (see the entry above, 2026-08-20) was left as-is rather than rebuilt as an address-text Google Maps search URL, since a working coordinate-based deep link already existed by the time this shipped; only its visible label was switched from city to street address. `StudioMapItemResponse`, `NearbyStudioResponse`, and `MyStudioResponse` were deliberately not widened — all three are list/pin-density surfaces that show city only on every benchmark competitor, matching the existing `/discover`/`/studios/map` precedent. **Known gap, flagged not fixed**: this adds a fourth client-side call site hitting Nominatim directly with no backend proxy, no server-set `User-Agent`, and no server-side rate limiting (the other three: `location-picker.tsx`'s `reverseGeocode`, `DiscoverPage.tsx`'s `reverseGeocode`/`handleLocationSearch`) — a small `GET /api/v1/geocode?q=...` proxy (real `User-Agent`, `public-read`-policy rate limiting, optional Redis cache) would fix all four at once but is cross-cutting and out of scope for this pass. **Backlog, not built**: an address-candidate dropdown (Nominatim `limit=5` instead of top-1-only) would narrow the gap versus Google Places/Mapbox-style autocomplete that every benchmark competitor (Vagaro/Fresha/Boulevard/Mindbody/Zenoti/GlossGenius) actually uses — `useAddressGeocode` ships top-1-only tonight. | Opposite framing from the NIPT precedent above, deliberately: collecting a full street address at onboarding and showing it (with directions) on the public studio page **is** the current vertical-booking-SaaS standard (CLAUDE.md rule 6) — all six benchmark competitors show a full address with a map/directions affordance on the studio detail page, not just a city name, unlike NIPT's local-compliance-only justification. Client-side-only Nominatim was kept for consistency with the three pre-existing call sites rather than introducing a one-off backend proxy for just this feature — a deliberate, flagged trade-off, not an oversight. Verified: `dotnet build`/`dotnet test` green (2396 unit tests); migration applied cleanly to a real local MySQL 8.4 instance; `pnpm vitest run` full suite green including new `useAddressGeocode.test.ts` and extended `RegisterStudioPage`/`StudioProfilePage`/`StudioPortfolioPage` suites; `pnpm lint`/`pnpm build` clean. |
| Live + offline admin notification on new studio registration (2026-09-16) | New studio registration (`RegisterStudioHandler`) now dispatches `SendStudioRegisteredNotificationCommand` inline via `ISender` (same pattern as `CreateAppointmentCommand` → `SendAppointmentCreatedNotificationCommand`), which emails every `admin`-role Identity account and writes one `NotificationLog` row (`RecipientType.Admin`, `RecipientId`/`StudioId` = the new studio's id, bypassing `INotificationPreferenceService` — a mandatory platform-ops notice, not a studio-opt-out-able one) plus a real-time push. **Terminology correction, not a re-litigation**: the source overnight prompt (`docs/claude/overnight-prompt-issuer-studio-registered-notification-2026-08-20.md`, dated 2026-08-20) was written before the Issuer→Admin rename (see "Issuer to Admin Rename" memory) and used "issuer"/`IssuerLayout`/`"platform:issuer-notifications"` throughout — none of that exists in the codebase anymore (role string is `"admin"`, layout is `AdminLayout.tsx`, policy is `"AdminOnly"`). Implemented against current reality: `NotificationRecipientType` gained `Admin` (5th value — the prompt assumed a 3-value enum; the real one already had a 4th, `ExternalContact`, from an unrelated prior feature). `NotificationHub` gained a **conditional auto-join** in `OnConnectedAsync` (join `"platform:admin-notifications"` iff the caller's role claim is `admin`) rather than a new hub, because every role already connects to `NotificationHub` for its own per-studio group — `TrafficHub`'s unconditional single-group auto-join (`AdminOnly`-gated, so unconditional is safe there) doesn't apply directly since `NotificationHub` is `[Authorize]` for all roles. `IIdentityService` gained one new method, `GetEmailsInRoleAsync(role, ct)` (thin `UserManager.GetUsersInRoleAsync` wrapper), rather than a new interface — it is this codebase's one established seam for Identity-backed lookups from the Application layer (`IAppDbContext` deliberately excludes Identity's `Users` DbSet). **Closed a real pre-existing bug as a side effect, not scope creep**: `GetNotificationsQuery` had no `admin`-role branch at all — an admin caller fell through to the generic `else if (query.RecipientId.HasValue)` arm, which silently narrowed every admin notification read to whichever single studio happened to be on the admin's own `tenant_id` JWT claim (harmless as long as nothing admin-facing had ever been written to `NotificationLog` — this feature is the first). Fixed with a new `admin` branch using `IgnoreQueryFilters()` (approved usage #53) returning every `RecipientType.Admin` row platform-wide, reapplying `DeletedAt == null` by hand since `IgnoreQueryFilters()` drops the whole combined filter, not just the `StudioId` half. `ResolveRecipientNamesAsync` merges `Admin`-type rows into the existing `Studio`-type lookup (both resolve `RecipientId` as a `Studio.Id`) rather than adding a second near-identical query. New index `ix_notification_logs_recipient_type` (migration `AddNotificationLogRecipientTypeIndex`) — the admin read has no `StudioId` in its `WHERE` clause, so none of `NotificationLog`'s three existing indexes help it. Frontend: new `useAdminNotificationHub.ts` (deliberately separate from `useSignalR.ts`, which is parameterized by `studioId` — an admin connection has no single tenant studio to key off), wired into `AdminLayout.tsx` (which already rendered `<NotificationBell />` but never established any live connection at all before this change). No onboarding-tour change — `adminTour.ts` has no existing step targeting the notification bell or `/platform/studios`, checked directly rather than assumed. Known, inherited (not introduced) risk, stated explicitly per this codebase's own flagging convention: if `IIdentityService.GetEmailsInRoleAsync` itself throws (not a per-recipient send failure, which is caught), the exception propagates out through the inline `sender.Send` call and fails the studio's own registration request — identical to how `CreateAppointmentCommand` already lets `SendAppointmentCreatedNotificationCommand` propagate uncaught with no try/catch at that call site either; not fixed here as it would be a scope-widening change to an already-established pattern, not a regression this feature introduces. | Mirrors the already-shipped `AppointmentCreated` notification pattern (persisted log + real-time push) applied to a platform-level event, per this project's "reuse the established seam" convention rather than inventing a parallel one. Verified: dotnet build/test green (2392 unit + all touched integration-test files green, including a new `SendStudioRegisteredNotificationHandlerTests` suite and extended `RegisterStudioHandlerTests`/`GetNotificationsHandlerTests`/`NotificationHubTests`/`NotificationDispatchTests`); migration applied cleanly to a real dev MySQL DB (local dev DB had unrelated drift — a same-named index left over from prior, unrelated work not tracked in `__EFMigrationsHistory` — dropped and the tracked migration reapplied cleanly, not worked around); `pnpm tsc --noEmit` clean; Help Menu (`admin-notifications` article) and standalone user manual (`#admin-notifications` section + nav link) both updated in the same change. |
| Studio address ↔ pin sync made bidirectional (2026-09-17) | Reverses the "One-directional by design" clause of the entry above (2026-09-16): dragging/clicking the map pin now also writes back into `addressLine1`, not just lat/lng/city. `location-picker.tsx`'s `reverseGeocode` gained a third return field, `streetAddress` (Nominatim's `address.road`/`pedestrian`/`footway` + `house_number`, joined), and `LocationPickerProps.onChange` widened to carry it alongside `lat`/`lng`/`city` — every existing consumer that ignores the new field keeps compiling and behaving identically (additive, not breaking). `RegisterStudioPage.tsx`/`StudioProfilePage.tsx` both only overwrite `addressLine1` when `streetAddress` is non-empty (a pin dropped somewhere with no resolvable road name — e.g. open countryside — never blanks out what the user already typed). Guarded against the obvious feedback loop with a `pinDrivenAddressUpdate` ref: set `true` synchronously right before the pin-driven `setValue("addressLine1", ...)`, passed as `useAddressGeocode`'s new `enabled: !ref.current` option for that one render, then reset to `false` in a `useEffect` keyed on the address value — without this, writing the reverse-geocoded address back into the field would immediately re-trigger the *forward*-geocode hook 700ms later, flashing "Locating on the map…" right after the user just finished manually correcting the pin, and potentially nudging the pin again if Nominatim's forward search for that exact text didn't resolve to the identical coordinate. | User-requested reversal of the prior session's explicit design call, not a bug fix — the original one-directional design was a deliberate, documented trade-off, but real usage showed the reverse direction (correct the pin, expect the address text to follow) is expected UX for a two-way-bound map/address pair, matching how this product category's competitors (Vagaro/Fresha/Boulevard/GlossGenius) implement pin-and-address sync. Verified: `pnpm tsc --noEmit` clean; `npx eslint` on every changed file clean (0 new errors/warnings); unit tests green including 4 new cases (2 per page: pin write-back fires, and a no-resolvable-road pin never clobbers a typed address) plus all 68 pre-existing `RegisterStudioPage`/`StudioProfilePage` tests unchanged; `frontend/e2e/critical-path.spec.ts` registration flow re-verified 6/6 passing on both themes (its mocked Nominatim reverse-geocode response has no `road`/`house_number`, so `streetAddress` resolves to `""` there and the guard correctly leaves the test's typed placeholder address untouched, requiring no e2e test changes). |
| Removed artists can be re-invited to any studio (2026-09-17) | Closed a real gap surfaced by a live staging bug: `DeleteArtistHandler` soft-deleted the `Artist` row but never touched the person's Identity account, so `CreateArtistCommand`'s cross-tenant reuse guard (added 2026-08-21, see the "Cross-tenant account reuse guard" entry) could never distinguish "still an active artist somewhere else" from "was removed and free" — a removed artist's account was permanently unable to join any studio again, direct-invite or solo-join-request alike. `DeleteArtistHandler` now calls the already-existing `IIdentityService.RemoveTenantClaimAsync(userId, studioId, ct)` after soft-deleting (it already clears both the `tenant_id` claim and the `ActiveTenantId` token if it pointed at that studio — no new Identity method needed). `CreateArtistCommand` gained a second reuse condition, `isFreedArtist` (`existingRoles.Contains("artist") && existingTenantIds.Count == 0`), alongside the existing `isOrphanedArtistForThisStudio` check; when it's the freed case, calls `EnsureTenantClaimAsync` for the new studio (the orphaned case already has the claim). `IJobScheduler.EnqueueArtistInvite`/`SendArtistInviteJob.SendAsync`/`IEmailRenderer.RenderArtistInvite` all gained a trailing `bool isRejoiningArtist = false` parameter (default-valued, so every pre-existing call site — including `ResendArtistInviteCommand`, which always means "new account, not yet activated" — compiles and behaves unchanged) so the invite email tells a rejoining artist they already have working credentials and can sign in immediately, offering the password-reset link as optional rather than implying (as the brand-new-account copy does) that setting one is required first. | Matches how every benchmark competitor (Vagaro/Fresha/Boulevard) treats artists moving between studios — a routine event, not a one-way door — per CLAUDE.md rule 6; the original reuse guard's tenant-isolation reasoning (CLAUDE.md rule 1) is fully preserved, since `isFreedArtist` only matches an account with *zero* tenant claims, never one still active at another studio. Verified: `dotnet build`/`dotnet test` green (2414 unit tests, up from 2408 pre-change); new `DeleteArtistHandlerTests` cases (tenant claim removed when `UserId` is set, not called when it's null) and `CreateArtistHandlerTests` cases (freed-artist reuse succeeds, grants the new tenant claim, enqueues the rejoining-copy email; unaffected new-artist path still enqueues the non-rejoining copy) all pass. Help Menu (`owner-artists-add` tip) and the standalone user manual (`/artists/new` section) both updated in the same change. |
| Owner/Artist header switcher for dual-role owners (2026-09-17) | User-reported gap: once an owner enables their own artist profile (see the "Owner-as-Artist" entry above), the only way back to it was a single "My Portfolio" nav item buried among 20+ other owner nav items — no clear, persistent way to jump between the owner dashboard and the artist profile. Deliberately did **not** build a full second layout/route-tree (rendering `ArtistLayout` for an owner) — `AppLayout` (`frontend/src/app/router.tsx`) is a hard single switch keyed on the JWT `role` claim, which never becomes `"artist"` for an owner by design (RBAC already treats `owner` as a superset via `ArtistAndAbove`), and `GetAppointmentsQuery`/etc. already support an explicit `?artistId=` filter for a non-artist caller — so the owner's existing `/artists/{id}` page (Profile/Portfolio/Schedule/Bookings/Designs/Social tabs, same page already used to manage any artist) already **is** a complete personal artist view once reached; the gap was discoverability, not missing functionality. New `ArtistModeSwitcher` (`frontend/src/shared/components/ArtistModeSwitcher.tsx`) — a small "Owner / Artist" segmented pill in `OwnerLayout`'s header, rendered only when `hasArtistProfile`, always visible at `lg`+ widths (paired with the existing `hidden lg:flex` desktop nav; mobile still uses the pre-existing "My Portfolio"/"My Earnings" `NavDrawer` entries, avoiding header overflow risk on small screens rather than adding new responsive plumbing for a redundant mobile path). Active-tab detection: "Artist" is selected when the current path starts with the owner's own `/artists/{myArtist.id}` (not any other artist's page) or is `/earnings`; everything else defaults to "Owner." New `owner-artist-mode-switch` onboarding-tour step (skipped gracefully pre-profile, same mechanism as the existing `owner-become-artist-cta` step already relies on). | Matches how vertical-booking-SaaS competitors (Vagaro/Fresha/Boulevard) surface a persistent "acting as" affordance for an owner who is also a bookable provider, rather than requiring them to hunt through a full settings-style nav for their own booking/portfolio page — CLAUDE.md rule 6. A true mode-switching second layout was considered and rejected as unnecessary scope: the owner already has full access to everything an artist needs (Deposit Rules, Waitlist, Messages, etc. all already on `OwnerLayout`'s own nav), so duplicating `ArtistLayout`'s route tree behind a client-side role override would add real complexity for no functional gain over making the existing page more discoverable. Verified: `dotnet build` n/a (frontend-only change); `pnpm vitest run` — new `ArtistModeSwitcher.test.tsx` (7 cases: renders both tabs, correct active-tab detection including the "different artist's own page ≠ my page" edge case, both navigation clicks) and extended `OwnerLayout.test.tsx` (4 new cases: hidden with no profile, shown with one, both navigation directions) all pass; Help Menu (`owner-become-artist` tip) and the standalone user manual (`#owner-become-artist` section) both updated in the same change; `docs/claude/architecture.md` Decisions Log updated here. |
| Owner's own tenant claim must never be freed on self-delete (2026-09-17) | A same-day follow-up fix to the "Removed artists can be re-invited to any studio" entry above, found live on staging within hours of that change deploying: `DeleteArtistHandler`'s new `RemoveTenantClaimAsync` call removed the tenant_id claim for ANY deleted artist's linked Identity account — correct for a genuinely invited artist, but a studio owner's own dual-role artist profile (`CreateOwnArtistProfileCommand`, "Stop working as an artist") links `Artist.UserId` to their own owner account, and their tenant_id claim on that studio is the exact same claim mechanism that grants their OWNER access to it. An owner who clicked "Stop working as an artist" had their only tenant_id claim wiped, losing every tenant-scoped page — not just their artist seat. Fixed by checking the linked account's role via `IIdentityService.GetUserRolesAsync` before calling `RemoveTenantClaimAsync`: only removed when the role is `"artist"`, never `"owner"`. This directly restores an invariant the original owner-as-artist dual-role spec explicitly documented and the fix broke without realizing it applied: *"`DeleteArtistHandler` only ever sets `artist.DeletedAt` — it never calls into `IIdentityService`. Removing the owner's linked profile can never lock them out of their own account."* | A same-day regression from moving too fast on the prior fix, not a design trade-off — the invariant was already documented, just not re-checked against the new code path. Confirmed exactly one staging account affected (repaired by hand via direct SQL); zero production accounts affected (checked: zero `artists` rows soft-deleted in `pena_e_arte_prod` since the deploy). Verified: `dotnet build`/`dotnet test` green (2416 unit tests); new `DeleteArtistHandlerTests` cases prove an owner-linked artist deletion does NOT call `RemoveTenantClaimAsync` (but still soft-deletes the `Artist` row correctly) while a genuinely-invited-artist deletion still does. |
| Owner/Artist header switcher widened below the `lg` breakpoint (2026-09-17) | The switcher above was gated `hidden lg:flex` (1024px+), matching the full desktop nav — but that meant it vanished entirely on any window narrower than 1024px, including an ordinary ~940px browser window where the header still had visible room for it (confirmed via a real screenshot). Widened to `hidden sm:flex` (640px+); true phone widths still fall back to the pre-existing "My Portfolio"/"My Earnings" `NavDrawer` entries. | Direct user-reported bug against the switcher shipped the same day — the original `lg` gate was a defensive default, not a considered trade-off; the header had room at `sm`+ all along. Verified: `pnpm vitest run` (24 `OwnerLayout.test.tsx` cases green, unaffected by the class-name-only change). |
| Owner/Artist nav is genuinely context-scoped, not just a link to a shared "full access" view (2026-09-17) | Direct follow-up to a user report against the switcher shipped hours earlier ("make sure the artist and the owner only has his own specific menus") — real screenshots showed the exact same full owner nav (Dashboard, Artists, Payments, Billing, Studio Settings, Promo Codes, Booth Rent, Gift Cards, Packages, Campaigns, studio-wide Conduct Reports…) whether the owner was in Owner mode or had switched into Artist mode. The switcher's own Decisions Log entry above reasoned "the owner already has full access to everything an artist needs" — true for *authorization*, but wrong for *what the menu should show*: "full access" means the owner's shared pages (Schedule, Designs, Reports About Me) render the whole studio's data by default, not "my own only," which is not the same thing a real artist sees on those same pages. Audited every route on `ArtistLayout`'s own nav against its backing query for hidden role-branching (all already-registered routes in the single shared route tree under `AppLayout`, per `frontend/src/app/router.tsx`'s own `// ── Shared: X` comments — no new routes needed): `GetClientsQuery`/intake+consent form list queries have zero role branching at all (genuinely shared, safe to reuse identically); `GetAppointmentsQuery`/`GetDesignsQuery` already accept an explicit `ArtistId` param honored for any non-artist-role caller (i.e. an owner) — `DesignListPage.tsx` already read `?artistId=` from the URL and forwarded it, `SchedulePage.tsx` did not, now does (mirrors the same pattern); `GetMyConductReportsAsArtistQuery`'s backend handler resolves purely by the caller's `UserId` with zero role check and its endpoint policy is `ArtistAndAbove` (already includes owner) — the only real bug was `ConductReportsPage.tsx` hard-branching its own view on `role === Role.Artist`, never considering an owner-in-artist-context; fixed by also treating `role === Owner && searchParams.has("artistId")` as the artist view. New shared `frontend/src/shared/utils/artistContext.ts` (`isInArtistContext`) used by both `ArtistModeSwitcher` (active-tab detection) and `OwnerLayout` (which nav-item-set to render) so the two stay in sync — an artist-mode nav link now carries `?artistId=<the owner's own artist id>` wherever a shared page needs it (Schedule, Designs, Reports About Me), and `OwnerLayout.artistNavItems` deliberately excludes every owner-only studio-management item, leaving exactly what `ArtistLayout`'s own `STATIC_NAV` shows a real artist (Schedule, Clients, Messages, Designs, Intake Forms, Consent Forms, Deposit Rules, Waitlist, My Earnings, Reports About Me, plus My Portfolio and an "Owner Dashboard" escape hatch back). No backend changes were needed anywhere — every query already supported this, it was purely unwired on the frontend. | Directly closes the gap CLAUDE.md rule 6 exists to catch — the switcher's first version shipped a nav that merely relabeled "acting as artist" without actually scoping what it showed, which is worse than not having the feature (implies a filtered view that wasn't really filtered). Verified: `dotnet build`/`dotnet test` n/a (frontend-only, zero backend changes); `pnpm tsc --noEmit`/`pnpm lint`/`pnpm build` all clean; `pnpm vitest run` — new `SchedulePage.test.tsx` cases (forwards/omits `?artistId=`), rewritten `ConductReportsPage.test.tsx` (now wrapped in `MemoryRouter`, two new cases for the owner-with-artistId path vs the plain owner path), and 5 new `OwnerLayout.test.tsx` cases (owner mode hides artist-only items, artist mode hides owner-only items, Schedule/Reports-About-Me links carry the right `?artistId=`, the "Owner Dashboard" escape hatch works) all pass; Help Menu (`owner-become-artist` tip) and the standalone user manual (`#owner-become-artist` section) both updated in the same change. |
| Client Account Deletion Hardening — cross-studio erasure fan-out fix, archive/restore, self-service export, erasure cancel (2026-09-18) | Engineering-consultation review of "delete a client's account" surfaced a real, shipped correctness bug: `ClientDataErasure.ExecuteAsync` (shared by owner-initiated `RequestDataErasureCommand` and client self-service `RequestMyDataErasureCommand`) only ever touched the single `Client` row resolved from the caller's active tenant, while `IIdentityService.DisableLoginAsync` disables the shared Identity login globally — a client belonging to more than one studio (a real, working feature; see "Client identity model" below) ended up locked out everywhere while their PII survived, un-anonymized, forever, at every studio but the one they happened to be active in. Fixed by fanning `ClientDataErasure.ExecuteAsync` out across every `Client` row sharing the caller's `UserId` (new `ClientAccountExtensions.FindAllClientRecordsForUserAsync`, extending approved `IgnoreQueryFilters()` usage #29 rather than adding a new row — same class of cross-tenant `UserId` lookup as its siblings) before disabling login once; `RetentionPurgeJob.AnonymizeErasedClientsAsync`'s existing cross-tenant sweep needed zero changes to pick up more than one marked row per run, confirmed by reading it rather than assumed. Three benchmark gaps closed in the same pass: (1) new `Client.ArchivedAt` (nullable, **not** part of any EF Core query filter, unlike `DeletedAt` — EF Core applies a related entity's own global query filter even when reached via an unfiltered parent's `Include()`, so reusing `DeletedAt` for "remove from list" would have silently nulled an archived client's `Client` navigation on every past `Appointment`/`Payment`/`ConsentForm`) backing new `ArchiveClientCommand`/`RestoreClientCommand` (`ArtistAndAbove`, mirrors `Deactivate`/`ReactivateReferralCodeCommand`'s "reversible, audited, owner-initiated toggle" shape) — the ordinary Fresha/Vagaro-style "delete client" that declutters the list while keeping every record fully intact, applied as an explicit opt-out `Where` in `GetClientsQuery` (new `IncludeArchived` param), never a query filter; (2) new self-service `ExportMyDataQuery` (`GET clients/me/export`, `ClientAndAbove`) using the same cross-tenant fan-out helper to return one `ClientDataExportResponse` bundle grouped by studio — a deliberate consistency call, not a flagged question, since shipping the export with the same active-tenant-only flaw Phase A had just fixed would have been building the same bug twice in one night; (3) new owner-facing `CancelDataErasureCommand` (`OwnerOnly`) undoing a pending erasure during the 30-day grace window by restoring only the rows that specific request soft-deleted (matched by the exact `ErasureRequestedAt` timestamp, so an unrelated row independently expired by the routine retention pass is never resurrected) and re-enabling login via new `IIdentityService.EnableLoginAsync` (clears the lockout end date `DisableLoginAsync` set; lockout *capability* stays enabled). Deliberately **owner-facing only, not self-service** — a client's login is disabled the moment erasure is requested, so they cannot log back in to cancel their own request by design; a genuine self-service undo would need a different design entirely (time-delayed lockout with an "are you sure" window *before* disabling login), logged as a backlog note below, not silently added to this pass's scope. **Client identity model, confirmed not assumed:** `Client` is a `TenantEntity` (one row per studio) but one Identity `UserId` can hold a `Client` row at more than one studio simultaneously — an existing, working feature ("My Studios," `SwitchStudioCommand`, `AcceptStudioJoinInviteCommand`), documented by `ClientConfiguration.cs`'s own `ix_clients_user_id` index comment and already covered by approved-usage row #29 before tonight. **Stale doc line corrected, not re-litigated:** the PENA-104 entry above (2026-07-31) claimed "No client-facing self-service erasure UI yet" — false since an undated later session shipped `RequestMyDataErasureCommand`/`DeleteAccountSection.tsx`; that row's trailing sentence is now corrected in place rather than left misleading a future session. **Backlog, spec'd but explicitly not built tonight** — a third fork, platform-level client suspension for abuse/fraud (proposed non-tenant `ClientSuspension` entity, distinct from both owner-initiated erasure and archive), requires product/legal decisions this pass wasn't authorized to make (does a suspension lock the shared login everywhere or only block new activity; is there an appeal path; does resolving a `ConductReport` ever auto-trigger one, or is it always a separate manual admin action; does a suspended client's existing data get touched at all) — confirmed via grep that no `BanClient`/`SuspendClient`/`AccountBanned` concept exists anywhere in `Domain`/`Application`/`API.Endpoints` today, a real gap, not a naming mismatch; do not infer answers from the benchmark set, none of Fresha/Vagaro/Boulevard/GlossGenius publicly document their own trust-and-safety internals in enough detail to safely copy. Also backlogged: a per-jurisdiction consent/health-record retention-floor table, relevant only once the platform supports non-Albania studios (today's flat 7-year `RetentionOptions` default is founder-confirmed 2026-08-01 and NIPT-gates registration to Albania-only, so no `Studio.Country` field exists or is needed yet). | GDPR Art. 17 itself, not a competitor benchmark, drives the Phase A fix — erasure means erasure, not erasure-at-one-of-several-locations a client happens to belong to. Phase B directly matches Fresha's own documented "delete client" behavior (removes from the active list, retains sales/appointments/reviews, audit-logged) — CLAUDE.md rule 6. Phase C matches the general GDPR Art. 20/CCPA "offer an export before/alongside deletion" expectation. Phase D closes this feature's own prior point-of-no-return gap; no direct competitor precedent was found or needed. Verified: `dotnet test` green (2434 unit + 492 integration, up from the pre-change baseline — new `ArchiveClientHandlerTests`/`RestoreClientHandlerTests`/`CancelDataErasureHandlerTests`/`ExportMyDataHandlerTests` plus extended `RequestDataErasureHandlerTests`/`RequestMyDataErasureHandlerTests`/`GetClientsHandlerTests`, and a new `ClientDeletionHardeningEndpointAuthorizationTests` integration suite proving the real ASP.NET Core auth pipeline against a real MySQL instance — a fake `DbContext` registers no query filters at all and cannot prove the cross-tenant fan-out actually crosses the tenant boundary); `pnpm tsc -b`/`pnpm lint`/`pnpm vitest run` all green (2308 tests, 3 pre-existing documented `StudioProfilePage.test.tsx` flakes unrelated to this change, re-confirmed by the file's own in-test comment dated 2026-09-05); `pnpm build`-equivalent (`tsc -b`) surfaced 10 pre-existing test-fixture literals across other features that needed a trailing `archivedAt: null` once `ClientResponse` gained the field — all fixed in the same change, none left broken. No onboarding-tour step added anywhere: `clientTour.ts` has no existing step targeting `MyProfilePage`'s Sharing tab at all (checked directly, not assumed), and `artistTour.ts`/`ownerTour.ts`'s existing "Your clients" nav steps point at the sidebar link, not page-internal UI — the row-level kebab menu and "Show archived" toggle were judged discoverable without a dedicated step, matching this file's existing convention of stating that reasoning explicitly rather than silently skipping the question. |
| Four-tier catalogue + yearly on every paid tier (2026-09-23) | Moved from five tiers to four (Free/Starter/Growth/Premium): Pro is retired — its `Plan` row is kept (subscriptions/history may reference it) but every `PlanPrice` row is set `IsActive = false`, and a fresh DB never gets a Pro row; Premium absorbs Pro's capacity limits and its one real feature, `AllowApiAccess`. `MaxLocations` is `null` on every core tier — `PlanLimitService` hardcodes usage to 1 (multi-location isn't built), so the field was never enforceable; the column and `QuotaType.Locations` plumbing stay, just unadvertised. Starter and Growth each gained a Yearly `PlanPrice` row (290/590) with `StripePriceId = null` — unlinked, so the subscribe page's `purchasablePriceFor` treats them as unavailable until a human links real Stripe prices after the Batch 3 refund flow ships. Referral coupon fix: a referral reward is now always worth exactly one month of the tier's Monthly price (`ReferralRewardAmount.OneMonthOf`), not a blanket "1 month free" coupon — the old `CreateOneMonthFreeCouponAsync` (100% off, repeating, 1 month) made a yearly-billed referral's entire first year free, since a yearly subscription has only one invoice inside that repeating window. Monthly stays on the old coupon; Yearly gets a fixed amount-off coupon (once) for the referred studio, and a Stripe customer-balance credit (applied to the next invoice) for a Yearly-billed referrer, since a coupon on a subscription up to a year from its next invoice would either expire unused or (percent-off) zero a whole renewal. The owner-facing yearly saving (`YearlySavingAmount`/`YearlyMonthsFree` on `PlanResponse`, `frontend/src/features/billing/utils/yearlySavingLabel.ts`) is computed from the two real `PlanPrice` rows everywhere it's shown (plan cards, the Monthly/Yearly toggle badge, the trial-expiry email); `Plan.YearlyDiscountPercent` is now admin-input-only, feeding just the plan editor's suggested-yearly-price helper ("months free on yearly" in the UI, still sent to the API as a percentage so the contract/validators stay unchanged). **Flagged, not fixed this pass:** owner-facing `GET /api/v1/billing/plans` still returns `SubscriberCount` and every price's `StripePriceId` to every owner (platform data leak); `ReferralRedemption.ReferrerRewardCouponId` now also holds a `cbtxn_…` id for a Yearly referrer, not just a coupon id (naming follow-up, no migration); live Stripe yearly prices for Starter/Growth and the yearly-refund flow are Batch 3. | The Industry-Standard Benchmark Set (GlossGenius, Boulevard, Square Appointments, Venue Ink, Fresha, Booksy, Vagaro) sells three or four flat-priced tiers separated by team size, with a modest (12–20%) yearly discount — Pro differed from Premium only by capacity and API access, so retiring it and folding its value into Premium matches that pattern (CLAUDE.md rule 6) better than five overlapping tiers. The coupon bug was a real, live correctness issue: a repeating 100%-off coupon applied to a yearly subscription's single in-window invoice zeroed the referred studio's entire first year, not the intended one month. |
| One MRR definition (2026-09-23) | D1–D5: the MRR KPI card (`GetPlatformStatsQuery`) and the MRR history chart (`GetMrrHistoryQuery`) now share one pure rules module (`MrrRules`) and one loader (`MrrInputLoader`) instead of each computing revenue independently. Headline MRR = Active subscriptions' monthly-equivalent price (Yearly ÷ 12); PastDue is reported separately as "at risk," not in headline MRR; Active + `CancelAtPeriodEnd` stays in headline MRR and is also reported as "scheduled churn." MRR for a past month is measured at that month's last moment (industry convention — ChartMogul/Baremetrics/Stripe Billing analytics), reconstructed conservatively from current subscription state (can under-count, never over-count) since there is no revenue-history table yet (that's Batch 3). ARPU (MRR ÷ active subscriptions, which counted Free studios paying €0) is replaced with ARPA (MRR ÷ paying studios). D5 blocks three plan-editing paths that could silently change reported revenue without changing what Stripe bills: `UpdatePlanHandler` deactivates rather than deletes a price interval with subscribers; rejects an in-place price/Stripe-price-id change on a price with subscribers; and both `CreatePlanHandler`/`UpdatePlanHandler` validate a linked Stripe price's amount/interval against the `PlanPrice` row via a new `IStripeBillingService.GetPriceAsync`. `DataSeeder.ReconcileCoreTiersAsync` no longer overwrites a linked (non-null `StripePriceId`) price row's `Price` on drift — it logs a warning instead. | The two admin-facing revenue figures disagreed with each other and the history chart counted trial months and post-cancellation months as revenue — neither matches the D2 month-end MRR convention every benchmark platform (ChartMogul, Baremetrics, Stripe Billing analytics) uses, and ARPU's Free-studio inflation is a real reporting bug. Plan edits changing reported revenue without a matching Stripe change is a correctness bug, not a hypothetical. |
| Suspension pauses billing (2026-09-23) | D6: `SuspendStudioCommand`/`UnsuspendStudioCommand` now call new `IStripeBillingService.PauseCollectionAsync`/`ResumeCollectionAsync` (best-effort, same try/catch-and-log pattern as `CancelSubscriptionHandler` — the DB suspension stands regardless of the Stripe call's outcome) for any card-billed, non-Cancelled subscription. `ResumeCollectionAsync` clears `pause_collection` via `SubscriptionUpdateOptions.AddExtraParam("pause_collection", "")` rather than the typed `PauseCollection` property, since Stripe.net 52.4.1 has no typed "unset" for that object-shaped param (a null value is simply omitted from the request, leaving the existing pause in place) — sending an empty string is the Stripe API's own documented way to clear an object parameter. `PastDueReminderJob` now skips suspended studios (`Studio.IsActive == false`) — their owner can't log in to pay, so a reminder email is pointless. Reporting: a suspended studio's revenue moves from headline MRR to "paused MRR" (see the MRR entry above) from its latest `Studio.Suspended` audit timestamp onward. | Suspension already blocked every login for the studio (`admin-suspend-studio` Help article), but its Stripe subscription kept renewing and charging — billing a locked-out customer is against standard B2B SaaS practice and invites card disputes; Stripe's `pause_collection` exists for exactly this case. |
| Yearly → Monthly waits for the paid year to end (2026-09-23) | D7: `ChangePlanHandler`'s upgrade/downgrade split (`MonthlyEquivalent(new) > MonthlyEquivalent(current)` ⇒ immediate + prorated) now excludes any switch from a Yearly-billed subscription to a Monthly price, whatever the tier — that combination is scheduled at period end like any other downgrade, via the existing `PendingPlanId`/`PendingBillingInterval` + `ScheduleSubscriptionPriceChangeAsync` path. `SubscribePage.tsx` shows "Starts when your paid year ends on {date}" under every Monthly plan card when the studio is card-billed and currently on Yearly. | The immediate-upgrade path used `ProrationBehavior = "always_invoice"`, which credits the unused part of the discounted yearly price to the customer's Stripe balance — a time-based refund at the yearly rate, as account credit, bypassing the agreed yearly-refund rule (used months charged at the monthly price) that Batch 3's revenue ledger will enforce properly. Premium Yearly (790/12 ≈ €65.83) → Premium Monthly (€79) was live today as a same-day "upgrade" under the old rule. |
| Subscription billed-amount snapshot (2026-09-23) | Batch 2b. Every MRR figure previously read `Plan.Prices.FirstOrDefault(pp => pp.Interval == s.BillingInterval).Price` at query time — the price list for *new* sales, not what an existing studio is actually billed. Five new nullable columns on `Subscription` (`BilledUnitAmount` decimal(10,2), `BilledQuantity`, `BilledCurrency` varchar(3), `RecurringDiscountPercent` decimal(5,2)) are populated/refreshed at every billing-relevant site — checkout activation, direct create, every `customer.subscription.updated` webhook event (not just a price-id change), manual cash activation, and demo provisioning — from `IStripeBillingService.GetPriceAsync`/the webhook payload/the already-validated `PlanPrice`. `MrrRules.MonthlyEquivalent` reads the snapshot first, excludes a non-platform-currency snapshot from MRR entirely (never converts), and falls back to the live `PlanPrice` lookup only while the snapshot is null (pre-backfill or a missed write) — `GetPlatformStatsHandler`/`GetMrrHistoryHandler` log the fallback and currency-excluded counts, never silently swallowed. New `SubscriptionInvoicePayment` (one row per paid Stripe invoice, upserted idempotently on `StripeInvoiceId` from `invoice.paid`) backs a new `DiscountsThisMonth` platform-stat figure (coupon discount + customer-balance credit consumed, summed for the current calendar month) — deliberately not a `TenantEntity`, same class as `Subscription` itself, and reusable by Batch 3's refund calc/revenue ledger without a second migration. Batch 2's G2 guard (`UpdatePlanHandler`) is relaxed from "reject any price/Stripe-price change with a subscriber" to "reject only while any current/pending subscriber on that price is still unsnapshotted" — since changing `PlanPrice` never touches Stripe, a subscriber with its own snapshot keeps billing at its original Stripe price regardless of what the row says afterward; only new sales read the changed row. New `AdminOnly` `POST /api/v1/platform/subscriptions/backfill-billed-amounts` (`BackfillSubscriptionBilledAmountsCommand`) snapshots every pre-existing subscription — card-billed from a new `IStripeBillingService.GetSubscriptionBilledPriceAsync`, cash-billed from the plan's Monthly price (returned in the response for admin review, since nothing is billed by the action itself) — idempotent, not an automatic startup step. Confirmed: no new `IgnoreQueryFilters()` registry row is needed — `SubscriptionInvoicePayment` reads go through `db.SubscriptionInvoicePayments` directly, same as `db.Subscriptions` today, with no filter to bypass. | Stripe Billing's own MRR metric is computed per-subscription-item from what that item is actually charged, immune to later catalogue price changes — reading the price list instead is the same bug class as the pre-July `Plan.PriceMonthly` overstatement (CLAUDE.md rule 6). No studio was subscribed to a paid plan when this shipped (confirmed in Batch 2's report), so the backfill only touches demo/test data — cheapest to ship before the first real subscriber. |
| Yearly cancellation refunds (2026-09-24) | Batch 3a. A studio cancelling a Yearly plan is refunded `AmountPaid − MonthsUsed × MonthlyReferencePrice` (floored at 0; a started month counts in full), computed once by `YearlyRefundCalculator`/`YearlyRefundQuote` and shared by the owner-facing quote (`GetCancellationQuoteQuery`) and both cancel paths so the number an owner sees before confirming is exactly what gets refunded. Schema: three new nullable columns on `SubscriptionInvoicePayment` (`PeriodStart`, `MonthlyReferencePrice` — Yearly invoices only, snapshotted so a later Monthly price change never retroactively changes an already-issued refund — and `StripePaymentIntentId`, the refund target), populated in `HandleInvoicePaidHandler`/the `invoice.paid` webhook case (`BillingEndpoints.cs`) from `invoice.Lines.Data[0].Period.Start` and `invoice.Payments.Data[0].Payment.PaymentIntentId` — the Stripe.net 52.4.1 post-"thin invoice" shape, verified by reflecting over the compiled SDK; `Invoice` no longer exposes `PaymentIntentId` directly at all in this version. New non-tenant `SubscriptionRefund` entity (same class as `Subscription`/`SubscriptionInvoicePayment` — admin needs a cross-tenant read for "Refunds this month"), one row per invoice (unique index — the DB-level half of the "exactly one Stripe refund per cancel" idempotency guarantee; the app-level half lives in `YearlyRefundIssuer`, shared by both cancel handlers to avoid a circular `Platform.Commands`↔`Billing.Commands` dependency), `RefundRule` (YearlyFormula/AdminFull/AdminNone) and `RefundStatus` (Pending/Succeeded/Failed) enums, both stored as strings (matches this codebase's existing enum-column convention, e.g. `ConductReport.Category`/`FeedbackReport.Status`). Owner cancel flow (`CancelMySubscriptionCommand`, `BillingEndpoints.cs`'s three new `/subscription/cancel*` routes) replaces Stripe-portal cancellation entirely — portal cancellation is turned off via the Stripe Dashboard customer-portal setting (test mode, done this batch; live mode repeats it at launch), not a second API-managed `Configuration` object. Yearly cancels immediately (`Status=Cancelled`, `CurrentPeriodEnd=now`); Monthly (card-billed) keeps the existing cancel-at-period-end/no-refund policy via new `IStripeBillingService.ScheduleCancellationAsync`/`UndoScheduledCancellationAsync`; cash-billed cancels immediately with no refund (a deliberate simplification — no background job exists to later flip a "pending cash cancellation," and there is zero real financial exposure today). Admin override (`CancelSubscriptionCommand` gained `Override`/`OverrideReason`) reuses the identical refund branch with `AdminFull`/`AdminNone` short-circuiting the formula; a reason is required by the validator whenever `Override` is set; `AdminNone` still writes a zero-amount `Succeeded` `SubscriptionRefund` row (the one place that records the admin's "no refund" decision against the specific invoice). **`MrrInputLoader`/`MrrRules.SubscriptionRevenueInput.AdminCancelledAt` renamed `CancelledAt`** and its audit-log lookup widened to include the new `AuditActions.SubscriptionCancelledByOwner` action (alongside the existing `SubscriptionCancelledByAdmin`) — an owner-initiated yearly cancellation now also ends access immediately, not at the original `CurrentPeriodEnd`, so `MrrRules`' D3 pre-ledger window-reconstruction needs the same precise-end treatment admin cancellations already had; `CurrentPeriodEnd=now` is set as a belt-and-suspenders fallback on every yearly cancellation so `BillingWindow` is correct even if the audit-log join is ever missed. Refund status webhook (`refund.updated`/`charge.refund.updated` — both handled, since this session had no live Stripe Dashboard access to confirm which this account's endpoint actually has configured) updates `SubscriptionRefund.Status` from Stripe's async outcome; a transition into `Failed` is logged (`ILogger.LogError`) and surfaced on `AdminStudioSummaryResponse.RecentRefunds` (last 5, admin studio-detail page) — satisfying A6's "alert the admin" requirement without paging. New `RefundsThisMonth` platform-stat (sum of `Succeeded` refunds created this calendar month) added as a fourth clause to the existing `mrrSubtitle` helper, matching `DiscountsThisMonth`'s own precedent rather than adding a new dashboard tile to an already-full grid. Confirmed: no new `IgnoreQueryFilters()` registry row needed (`SubscriptionRefund` reads go through `db.SubscriptionRefunds` directly, same as its siblings). Deliberately deferred, CLAUDE.md rule #7 exception (see this file's "In-App Help Menu" section and the source overnight prompt's §11): `helpContent.ts`/the standalone manual are **not** updated with the refund-rule copy this batch, since Starter/Growth Yearly aren't purchasable yet and the terms need legal review before publication — only a one-line dated `TODO` was left on the `owner-subscribe` article; the in-app UI copy the feature needs to function (the cancellation-quote dialog, the confirmation email) ships normally, since without it the feature wouldn't work at all. Batch 3b (the revenue ledger, `SubscriptionRevenueEvent`/MRR-movements/GRR-NRR) can add a `Churn` ledger event at the exact moment `Status` flips to `Cancelled` in either cancel handler without any further schema change here — this batch only fixed `MrrRules`' existing D3 reconstruction via `CurrentPeriodEnd`/`CancelledAt`, a bugfix to code Batch 3b will later replace outright, not an implementation of it. | A time-based/prorated refund on annual-plan cancellation is unusually generous for this category — most vertical-booking SaaS (Vagaro/Fresha-tier) offer no refund path for annual plans at all — making the formula (charge used months at the monthly rate, keep the two free months' value for a stayer) a differentiator worth the UX investment in a clear pre-confirmation quote, which itself matches Stripe's own subscription-cancellation UX guidance of previewing financial consequences before an irreversible action (CLAUDE.md rule 6). |
| Subscription revenue ledger (2026-09-24) | Batch 3b. `GetMrrHistoryHandler` reconstructed every past month from CURRENT subscription state (`MrrInputLoader`/`MrrRules.BillingWindow`, D3: can under-count, never over-count) and so could not see a plan change or a since-corrected mistake that happened and reversed inside the window. New append-only, non-tenant `SubscriptionRevenueEvent` table (`subscription_revenue_events`: SubscriptionId, StudioId, OccurredAt, `RevenueEventType` New/Expansion/Contraction/Churn/Reactivation/Paused/Resumed/PastDue/Recovered stored as a string, PlanId, Interval, MrrBefore, MrrAfter, Source, StripeEventId, CreatedAt; migration `AddSubscriptionRevenueLedger`) written at the moment each MRR-affecting transition actually happens, so months on or after the ledger's first event are RECORDED history and earlier months stay an estimate (`MrrDataPointResponse.IsEstimated`; the chart draws estimated points hollow with dashed segments and only shows the caption when at least one visible point is estimated). **One writer.** `RevenueEventRecorder.Record` (co-located with `MrrRules`) skips a no-op (`MrrBefore == MrrAfter`) for the five amount types but always records the four state-only types, and builds the idempotency key: the real Stripe event id for webhook sources, else `Source:SubscriptionId:Type:OccurredAt` — the unique index on `StripeEventId` needs a non-null value on every row (MySQL has no filtered unique index). Its `RecordActivationAsync` decides New vs Reactivation from the ledger itself (`AnyAsync` by subscription id), never from `Status`, and turns an activation of an already-billing subscription into Expansion/Contraction; activation `MrrBefore` is 0 unless the subscription was already Active (`MrrBeforeActivation`), because a Cancelled/Trialing row keeps its old plan and billed-amount fields and would otherwise read as a fake contraction. **The one non-obvious modeling choice:** `MrrBefore`/`MrrAfter` are the CONTRACTED monthly-equivalent MRR (`MrrRules.MonthlyEquivalent`, reading the Batch 2b billed-amount snapshot, never the price list). `Churn`'s `MrrAfter` is always 0 (the contract ends); `Paused`/`PastDue`'s `MrrAfter` equals `MrrBefore` (the contract is unchanged, only its billing inclusion changes, derived from Type at query time by `RevenueLedgerRules.MrrAt`, whose billing types are New/Expansion/Contraction/Reactivation/Recovered/Resumed — the spec's own code omitted Resumed but its test table requires a Resumed row to count, so Resumed is included). That is what lets Resumed/Recovered hand back the exact figure Paused/PastDue set aside with no separate lookup. `RevenueLedgerRules.MrrAt(events, now)` equals `MrrRules.MrrAt(inputs, now, now)` for an up-to-date ledger (asserted to the cent in `RevenueLedgerRulesTests`); the headline `Mrr`/`AtRiskMrr`/`PausedMrr` stat fields deliberately STILL read `MrrRules` — only history moved to the ledger. **Write sites** (every one fires once per real transition and zero times on a no-op resend): `ActivateCheckoutSubscriptionHandler`, `CreateSubscriptionHandler`, `ActivateSubscriptionManuallyHandler` (New/Reactivation), `ChangePlanHandler` immediate-upgrade branch only (Expansion), `HandleSubscriptionUpdatedHandler` (PastDue/Recovered on a status transition, Expansion/Contraction only when a SCHEDULED change lands — `pendingWasSet && PendingPlanId is null` after the existing clearing block, so the `subscription.updated` echo of an immediate upgrade, where nothing was pending, writes nothing and the upgrade yields exactly one Expansion), `HandleSubscriptionDeletedHandler`, admin `CancelSubscriptionHandler`, owner `CancelMySubscriptionHandler`, `GracePeriodEndJob` (Churn only if a `SubscriptionInvoicePayment` exists — a never-converted trial churns nothing), `SuspendStudioHandler`/`UnsuspendStudioHandler` (Paused/Resumed). Churn is only ever written for a subscription whose previous status was Active or PastDue, so the `customer.subscription.deleted` echo of an immediate admin/owner cancel (row already Cancelled) and a webhook redelivery both write nothing; the same guard is applied to a `status=canceled` `subscription.updated`, so whichever webhook first flips the row records the churn. Deviations from the source prompt, made to keep the ledger equal to `MrrRules`: (1) a cash-billed owner cancel (no Stripe subscription, so no deleted webhook will ever arrive) records its Churn immediately, not only the yearly branch; (2) Paused is written only for an Active subscription with MRR above zero whose latest ledger row is not already Paused, and Resumed only when the subscription is still Active and its latest row is Paused — otherwise suspending a cancelled or PastDue subscription and unsuspending it would resurrect a churned contract as billing. **New reads.** `GetMrrMovementsQuery` (monthly New/Expansion/Reactivation/Contraction/Churn/Net, no estimated fallback — pre-ledger months read as zeros) and `GetRevenueRetentionQuery` (GRR/NRR as fractions; null when start MRR is 0; **measures the last COMPLETED calendar month, not the month in progress** — changed 2026-09-25 because a ratio over a partial month reads near 100% early and drifts down, and every recognised SaaS metric tool measures a closed period; the response carries `PeriodStart` so the tile is labelled with the month name, and the tiles show "—" until a full month of ledger exists) plus a stacked-bar `MrrMovementsChart` and two KPI tiles on `/platform`. Refinement over the prompt's literal formula: retention movements are restricted to the cohort of subscriptions already billing at the end of the previous month (`RevenueLedgerRules.BillingSubscriptionsAt`), so a subscription that started AND expanded within the same month cannot inflate NRR — matching the stated intent, existing customers only. **Backfill.** `POST /api/v1/platform/subscriptions/backfill-revenue-ledger` (`BackfillRevenueLedgerCommand`, AdminOnly, run from the Subscription Oversight page's "Data maintenance" card together with the billed-amount backfill, in that order — the ledger copies the billed-amount snapshot into MrrAfter. The card asks for an inline confirm and shows the returned counts. Both backfill commands are `IAuditableCommand`s (added 2026-09-24 after the Data maintenance card made them one click): actions `Platform.BilledAmountsBackfilled` / `Platform.RevenueLedgerBackfilled`, new `AuditTargetTypes.Platform` with `TargetId = Guid.Empty` (platform-wide, no `StudioId`), metadata = run counts only (the handler sets them on the command, read back by `AuditLogBehavior` like `CancelSubscriptionCommand`). Deliberately NOT `Subscription.*` actions with target type `Subscription`: `MrrInputLoader` reads cancellation/suspension timestamps out of the audit table by exactly that action + target type, and a backfill must never be mistaken for a cancellation. The Audit Log page shows target `Platform` without the all-zero id) writes one `New` per currently-Active, MRR-above-zero subscription with no ledger row at `MrrRules.BillingWindow(...).Start`, source `Backfill`; idempotent because any existing row excludes the subscription. An Active subscription of a suspended studio also gets a `Paused` (start + 1 tick when no suspension timestamp is known, to avoid an OccurredAt tie) so the ledger agrees with `MrrRules` and a later unsuspend has something to resume. **Flags, accepted as gaps (not silently decided).** (a) A bare Stripe Dashboard price edit on a live subscription with no pending change and no status transition updates `BilledUnitAmount` via the existing webhook snapshot logic but writes NO ledger event. (b) `ChangePlanHandler`'s Expansion uses the NEW plan's LIST price under the subscription's existing quantity and recurring discount, because `BilledUnitAmount` is only refreshed by the following webhook (which is deliberately a no-op for the ledger to avoid a double write); if Stripe ever charged something other than the list price that one event's `MrrAfter` would stay off with no later correction — accepted rather than adding a synchronous `GetPriceAsync` round-trip to the upgrade path, since G3 guarantees list price equals Stripe price. (c) The KPI tiles use the CURRENT calendar month so far, matching `newStudiosThisMonth`/`discountsThisMonth`/`refundsThisMonth`; if Finance wants the last full completed month instead it is a one-line change. (d) The synthetic idempotency key is coarse (source + subscription + type + instant): two distinct real events of the same type for the same subscription at the identical OccurredAt would collide on the unique index, and the key only dedupes when a caller passes a fixed `occurredAt` (the backfill does, so its rerun is protected by both the no-existing-row guard and the key) — not possible today since every write site is one user action or one webhook, but re-verify if a future change adds bulk subscription operations. (e) A `Trialing` to `Active` transition delivered only by a `subscription.updated` webhook (without `checkout.session.completed`) writes no New — the checkout/create/manual activation handlers are the supported activation paths. Confirmed: no new `IgnoreQueryFilters()` registry row is needed — `SubscriptionRevenueEvent` is deliberately not a `TenantEntity` and is read directly through `db.SubscriptionRevenueEvents` by admin-only handlers; the backfill's `MrrInputLoader` call reuses the already-approved usage #4. | Computing MRR from an append-only EVENT history with a New/Expansion/Contraction/Churn/Reactivation taxonomy, and month-over-month GRR/NRR derived from it, is exactly how ChartMogul, Baremetrics and Stripe's own Billing analytics model subscription revenue — reconstructing history from current state is the gap this closes, the same category of fix Batch 2b made for per-subscription billed amounts versus the price list (CLAUDE.md rule 6, admin/platform-tier billing analytics). |
| Real Stripe test-mode verification of the invoice/webhook paths (2026-09-24/25) | The billing webhook paths had only ever run against hand-built commands and reflected SDK shapes. A local run against REAL Stripe test-mode payloads (Stripe CLI `listen` forwarding to the local API, a test clock to advance real renewals, `pm_card_chargeCustomerFail` for a real decline; API version 2026-05-27.dahlia) verified the ledger write paths and found three real bugs, all fixed in the same change. **Verified correct as designed:** an immediate upgrade writes exactly one Expansion (from `ChangePlanHandler`) and Stripe's real `subscription.updated` echo writes nothing while refreshing the billed amount (so the list-price approximation matched Stripe's price); scheduling a downgrade writes nothing; the scheduled change landing on renewal writes exactly one Contraction keyed to the real event id and clears the pending plan; a failed renewal writes exactly one PastDue despite three `subscription.updated` events during the failure; an admin cancel writes one synchronous Churn and the real `customer.subscription.deleted` echo adds none. **Bug 1 (Batch 3a, real-money):** `invoice.paid` never yields a PaymentIntent. `Invoice.payments` is an expandable field that Stripe omits from webhook payloads and from a plain retrieve, so `invoice.Payments?.Data?...PaymentIntentId` was always null and no Yearly invoice ever recorded its refund target (a Yearly cancellation would have quoted no refund). `HandleInvoicePaidHandler` now resolves it for Yearly invoices via the new `IStripeBillingService.GetInvoicePaymentIntentIdAsync`, which fetches the invoice with `expand[]=payments`; best-effort, a failure logs a warning and never fails the webhook. **Bug 2 (Batch 3a):** `invoice.PeriodEnd` is the END OF THE PREVIOUS period on a renewal invoice (observed: invoice 09-24 to 10-24, line item and subscription 10-24 to 11-24), and the handler stamped it over the correct `CurrentPeriodEnd` that `subscription.updated` had just delivered, so every renewal left a stale date (wrong renewal date, and a wrong access end for a later cancellation). The endpoint now uses the line items' period end (`InvoicePeriodRules.CurrentPeriodEnd`). **Bug 3 (Batch 3b):** `HandleInvoicePaidHandler` unconditionally sets Status Active, so when `invoice.paid` beat `subscription.updated` the update handler never saw PastDue to Active and no Recovered was written, leaving a paying subscription excluded from ledger MRR indefinitely. Now whichever handler flips PastDue to Active records the Recovered. **Consequence of the fix, found in the same run:** the two webhooks ran CONCURRENTLY (same second), each read the row as PastDue before either committed, and both wrote a Recovered. Recovered is not a movement and `MrrAt` takes the latest row, so it is harmless, but the same pattern can duplicate a movement type (checkout finalize plus its webhook both writing New; a `canceled` update plus the `deleted` event both writing Churn). The ledger stays append-only, so `RevenueLedgerRules.MovementsFor` now counts rows for the same subscription, type and amounts within 60 seconds as one movement. A DB-enforced fix (a transition anchor in a unique key, plus a save-retry helper) was deliberately not built here; it needs a migration and touches every write site. **Fixed in a follow-up (2026-09-25): the first invoice was silently dropped.** The `invoice.paid` for a NEW subscription is emitted at the same moment as `checkout.session.completed` and can reach the API before `StripeSubscriptionId` has been stored; `HandleInvoicePaidHandler` then finds no subscription and returns, and Stripe does not retry a 200. For a Yearly plan that is exactly the invoice a cancellation refund is quoted from. `ActivateCheckoutSubscriptionHandler` AND `CreateSubscriptionHandler` (the direct-create path has the same ordering hazard: it creates the Stripe subscription, then saves the id) now call `LatestInvoiceRecorder.RecordAsync` last, which fetches the subscription's latest PAID invoice (`GetLatestPaidInvoiceAsync`, list with `expand[]=data.payments`) and records it through `HandleInvoicePaidCommand`; idempotent by `StripeInvoiceId`, so it is safe alongside the webhook whichever wins, and best-effort (a failure logs and never fails the activation). The invoice-to-command mapping now lives in one shared `StripeInvoiceMapper` used by both the webhook and this path. **Verified live against real Stripe (second session):** a real `POST /billing/subscription` (Premium Yearly) recorded exactly ONE invoice-payment row with a real `pi_` (the expand path works on a list call) and the webhook's `invoice.paid` deduplicated; the owner cancel quote was 706.00 (785 paid minus 1 started month x 79) and the cancel issued a real Stripe refund of 706.00 that came back `succeeded`, with the app's `SubscriptionRefund` row `Succeeded` — both `refund.updated` and `charge.refund.updated` are delivered by this API version and both were handled without a double refund; suspend set `pause_collection {behavior: void}` on the Stripe subscription and unsuspend cleared it to null (the raw `pause_collection=""` unset works), with Paused/Resumed ledger rows; a Yearly to Monthly switch created a Stripe subscription schedule, left the price yearly, created no proration invoice and no customer credit, and wrote no ledger row. **Bug 4 found in that session (Batch 2b, DiscountAmount):** the customer-balance-credit sign was inverted. The code returned 0 when `StartingBalance < EndingBalance`, but applying credit moves a negative balance TOWARD zero, so a consumed credit is `ending - starting`. A real invoice (subtotal 79000, starting -500, ending 0, amount_paid 78500) recorded `DiscountAmount = 0.00`; it now records 5.00, and only a starting balance that is a credit counts. Feeds the 'Discounts this month' stat and never touched MRR. **Remaining known gap:** concurrent handlers can still write duplicate ledger rows (only collapsed at query time, see above). **Not verified:** event redelivery (`events resend`) against the local forwarder, the case where the webhook genuinely beats the link (the recorder is unit-tested, and its idempotency against the webhook was observed live, but the losing-race ordering was not forced), a coupon-based discount, and the Stripe Customer Portal cancellation toggle (a dashboard setting). All Stripe objects were test-mode and were deleted afterwards (test clocks cascade to their customers and subscriptions). | Verifying payment-provider integrations against the provider's real sandbox payloads, not just mocks, is the standard for billing correctness in vertical booking SaaS (CLAUDE.md rule 6): both Batch 3a bugs passed every unit test and would only have surfaced with the first real Yearly customer. |
| Real visitor IP lost at the ingress — fixed for prod + staging (2026-09-25) | Found while diagnosing Live Traffic showing Location "Unknown" on prod. The GeoIP side was healthy (weekly `geoipupdate` CronJob succeeded, both `.mmdb` files on the PVC and mounted, config paths set, no startup warnings); the visitor's public address never reached the API. K3s's bundled Traefik Service was `externalTrafficPolicy: Cluster`, which SNATs every connection to an internal address, and Traefik had no forwarded-headers trust config, so the frontend nginx log showed only private addresses in both the peer and `X-Forwarded-For`. Consequences: `GeoIpService.Lookup` (which skips private ranges by design) always returned null, and the IP-keyed rate limiter and traffic IP hash saw ONE shared address for every visitor. Fix, two halves. (1) `k8s/cluster/traefik-helmchartconfig.yaml` (K3s `HelmChartConfig`, applied by hand — CD does not apply cluster-level objects): `service.spec.externalTrafficPolicy: Local` (safe because the cluster is a single node; revisit if a node is added) and `ports.web/websecure.forwardedHeaders.trustedIPs` = Cloudflare's published edge ranges, so Traefik keeps the `X-Forwarded-For` Cloudflare sends and still overwrites one sent by a direct non-Cloudflare client. One Traefik serves both `app.tattooos.co` and `staging.tattooos.co`, so this single change fixes both environments. (2) `ForwardedHeadersOptionsBuilder` gained `ForwardedHeaders:ExtraTrustedCidrs` (comma/semicolon/whitespace-separated CIDR list, non-secret, set to the same Cloudflare ranges in `k8s/base/api-configmap.yaml`) and `ForwardedHeaders:ForwardLimit` (default 2, set to 3): the chain the API now sees is `client, cloudflare-edge, traefik-pod` from the nginx pod, and the middleware stops at the first untrusted address, so without the Cloudflare CIDRs `RemoteIpAddress` would resolve to Cloudflare's edge, not the visitor. Tested against the real middleware in `ForwardedHeadersTests` (real client resolved; a client-supplied leading `X-Forwarded-For` entry is not honored; without the extra CIDRs it stops at the edge). Ranges come from cloudflare.com/ips-v4 and /ips-v6 and change rarely; update both files together. Rollout order mattered: the API change is a no-op under the old broken chain so it shipped first, then the Traefik override was applied. | A reverse-proxy chain that loses the client address silently disables per-client rate limiting and geolocation, and both the rate limiter and analytics looked fine in tests because each test supplied the header directly; only reading the real ingress logs showed it. Keeping the trusted proxy list explicit and in the repo, and only honoring forwarded headers from known proxies, is the standard for apps behind a CDN (CLAUDE.md rule 6). |
| SignalR across multiple API replicas — WebSockets-only + Redis backplane (2026-09-25) | Found via Live Traffic showing "Offline" on prod while staging showed "Live". Prod runs 2 API replicas (`k8s/base/api-deployment.yaml`), staging 1. SignalR's default connect is `POST /hubs/x/negotiate` then the real connection request, and the second must reach the SAME server as the first; behind the Cloudflare -> Traefik -> nginx -> Service chain each request is load-balanced independently, so the API logs showed negotiate 200 on pod A followed by `GET/POST /hubs/x` 404 on pod B, on every hub (traffic, notification, ...) — the client only connected when a retry happened to land on the right pod. And `AddSignalR()` had no backplane, so even a connected client on pod A would never receive a message sent from pod B (chat, notifications, schedule/design pushes) — a silent-loss bug, not just a badge. Two-part fix. (1) Frontend: every hook now builds its connection through `frontend/src/shared/signalr/createHubConnection.ts`, which in PRODUCTION builds connects over WebSockets only with `skipNegotiation: true`, so no server affinity is needed; dev keeps default negotiation because Vite's dev proxy doesn't reliably forward WebSocket upgrades. (2) API: `AddSignalR().AddStackExchangeRedis(...)` (package `Microsoft.AspNetCore.SignalR.StackExchangeRedis`) on the existing Redis with channel prefix `pena-e-arte:signalr`, so hub messages fan out across replicas. Rejected: sticky sessions (would need affinity at Traefik AND at the nginx -> API hop, fragile), Azure SignalR Service (paid external dependency, not needed at two replicas). Known trade-offs: a network that blocks WebSockets no longer falls back to SSE/long-polling in production; SignalR now depends on Redis being reachable (it already did for presence and rate limits). Verified on the real cluster after deploy (see the session notes), not just by unit tests: the backplane cannot be exercised without a Redis and two servers. | Multi-replica SignalR needs either stickiness or WebSockets-only plus a backplane; Microsoft's own hosting/scale guidance names exactly these, and a backplane is the standard for any real-time feature (chat, notifications) once a second server exists (CLAUDE.md rule 6). |
| Staging now has its own GeoIP files (2026-09-25) | Follow-up to the visitor-IP fix above, requested explicitly for both environments. `k8s/overlays/staging/` gained `geoip-pvc.yaml`, `geoip-cronjob.yaml` (identical to production's, same MaxMind account), `geoip-volume-patch.yaml` and the `GeoIp__*` paths in its `api-config-patch.yaml`; `cd.yml`'s staging secret step now also sets `GeoIpUpdate__AccountId`/`LicenseKey` from the same `MAXMIND_ACCOUNT_ID`/`MAXMIND_LICENSE_KEY` GitHub secrets production uses. This supersedes the earlier "staging intentionally has no equivalent" wording in the K8s .mmdb population entry. The CronJob only runs weekly (Wednesday 06:00), so the first population after a fresh deploy is triggered by hand with `kubectl -n tattooos-staging create job --from=cronjob/tattooos-geoip-update tattooos-geoip-update-first-run`. Until that job completes, staging Live Traffic keeps showing Location "Unknown" (GeoIpService degrades to null). Keep the two overlays' GeoIP files in sync. | Staging is where real-IP and location behavior gets verified before production, so leaving it permanently unable to resolve locations meant the last verification step could only ever run on production (CLAUDE.md rule 6: parity between the pre-production and production environments). |
...
| Studio structured hours + timezone handling — P1 Group 2 (2026-09-09) | **Phase 1 (hours):** new `StudioHours` entity mirroring `ArtistSchedule` (`IsAvailable`→`IsOpen`), hard-gated into `ArtistAvailabilityExtensions.IsAnyArtistAvailableAsync`/`CheckArtistScheduleAsync` right after the existing `StudioClosures` check; `{id}/hours` GET (`ClientAndAbove`)/PUT (`OwnerOnly`) endpoints; default Mon–Fri 09:00–18:00 seeded at registration in **both** `RegisterStudioHandler` *and* `RegisterSoloArtistCommand` (the prompt only named the former — the solo-artist path creates a Studio too and would otherwise ship permanently unbookable; also seeded in `DataSeeder`'s two demo studios for fresh dev/CI databases) plus every existing test fixture that seeds a Studio+ArtistSchedule (9 files); `openingHoursSpecification` added to `StudioPortfolioPage`'s JSON-LD and a visible hours block in its sidebar. **Phase 2 (timezone):** `Studio.Timezone` (default `"Europe/Tirane"`), owner-editable via a native-`Intl.supportedValuesOf`-populated `<Select>` (no new package), validated with `TimeZoneInfo.TryFindSystemTimeZoneById`; conversion centralized in a new `Pena_e_Arte.Application.Common.TimezoneUtils.ToStudioLocal` — **not** threaded into `IEmailRenderer`'s method signatures as the prompt suggested, since the three appointment-notification command handlers build their own subject-line and SMS-body date strings inline (not solely through `EmailRenderer`), so the conversion happens once per handler at the point `appointment.Date` is first used, covering the email body, subject, and SMS from one call; frontend: `withStudioTimeZone` helper applied to `AppointmentCard.tsx`, `AppointmentDetailPage.tsx`, and `MyEarningsPage.tsx` (the three of the prompt's six-file list that actually render an appointment *instant*). **Explicit deviations from the prompt's six-file list:** `SchedulePage.tsx`'s two `toLocaleDateString` calls format calendar week/day-navigation labels built from `new Date()`, not an appointment UTC timestamp — left unconverted, since applying a studio timeZone to a browser-local calendar boundary risks shifting the displayed day, not fixing anything. `RevenueTrendChart.tsx`'s `fmtMonth` formats a `"YYYY-MM"` aggregate bucket label (no time-of-day component) — converting it risks rolling the displayed month backward/forward depending on the studio-vs-browser offset, the exact "wrong value forced through" failure mode the prompt itself warned against; left unconverted. `MyBookingsSection.tsx` is confirmed unreachable without a larger data-plumbing change: `GetMyAppointmentsQuery`/`AppointmentResponse` carry no studio context, and a client's bookings can legitimately span several different studios' timezones — recorded as a real follow-up, not forced through. New test files (none existed before this pass): `ArtistAvailabilityExtensionsTests.cs`, `TimezoneUtilsTests.cs`, `StudioHoursEndpointAuthorizationTests.cs`, `UpsertStudioHoursCommandTests.cs`, `StudioHoursCard.test.tsx`; extended `GetPublicStudioHandlerTests.cs`, `UpdateMyStudioValidatorTests.cs`, `SendAppointmentConfirmationHandlerTests.cs`, `SetupChecklist.test.tsx`/`.tsx`, `StudioPortfolioPage.test.tsx`, plus a `TimezoneDataAvailabilityTests.cs` smoke test guarding the Alpine-vs-Debian tzdata risk. `SetupChecklist.tsx`'s new "Set your hours" item tracks "hours rows exist," same simplification its other two items already use — since every studio has rows from registration onward, this item is effectively always done immediately, a known, explicitly-accepted limitation rather than a silent gap. `EmailRenderer.cs`/`IEmailRenderer.cs` were **not** touched (a deliberate simplification over the prompt's literal suggestion — see above). 1971 backend unit + 402 integration tests green; frontend `tsc -b` clean, 2054+ vitest tests green. | Current vertical-booking-SaaS standard (CLAUDE.md rule 6) — structured hours with JSON-LD opening-hours markup is baseline for local-business discoverability (Fresha/Vagaro/Boulevard-class studio profiles all surface this), and correct local-time display is a correctness baseline, not a feature, for any booking product serving guest-artist/tattoo-tourism traffic across timezones. `StudioHours` existing as a real, enforced gate is a precedent later Group-3/4 items may want to cite for "when can a client interact with this studio at all" — noted here, not invented as a connection unless a later spec actually draws on it. |
