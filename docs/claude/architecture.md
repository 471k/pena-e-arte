...
| Pomelo.EntityFrameworkCore.MySql 9.0.0 / EF Core 10.0.10 mismatch blocks local `dotnet run` | Discovered 2026-07-26 while trying to bring the real `Pena_e_Arte.API` up locally to verify the observability stack end-to-end: `dotnet run` throws `MissingMethodException` in `UseMySql` at DI-configuration time (`AbstractionsStrings.ArgumentIsEmpty` signature changed between EF Core 9 and 10), unconditionally, since `Program.cs` calls `Database.MigrateAsync()` immediately on startup with no way to skip it. `dotnet build`/`dotnet list package` already surface this as `NU1608`: `Pomelo.EntityFrameworkCore.MySql 9.0.0 requires Microsoft.EntityFrameworkCore.Relational >= 9.0.0 && <= 9.0.999 but version 10.0.10 was resolved`. Not fixed as part of this change — package-version changes are outside this prompt's file scope, and this is already a known, tracked, accepted-as-blocked issue (Dependabot PRs #27/#28 for the EF Core 10 bump are held open pending a Pomelo 10.x release). Worked around for tonight's verification via a standalone throwaway harness (not committed) reusing the real `AddApiOpenTelemetry`/`RequestIdMiddleware` code with zero DB dependency — sufficient to verify metrics/traces/logs end-to-end, but the actual `api` container/binary itself was never smoke-tested tonight. | This means the real `api` service in `docker-compose.yml` cannot currently start at all, containerized or via `dotnet run` — a pre-existing condition, not something this change introduced or worsened, but worth surfacing loudly rather than letting an observability-stack PR imply the app was confirmed running end-to-end when it wasn't. Whoever picks up the Pomelo 10.x bump should re-run this prompt's Phase 2/7 verification steps against the real `api` service once it can start, to confirm the harness's findings (trace/log ID mismatch, real metric names) still hold against the actual app. |
