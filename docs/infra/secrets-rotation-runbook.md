...
| Cloudflare R2 access key | `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` | Cloudflare Dashboard → R2 → Manage API tokens | Create a new token, deploy, then delete the old token. `R2_ACCOUNT_ID`/`R2_BUCKET_NAME`/`R2_PUBLIC_URL` are not secret. This token is scoped only to the primary `pena-e-arte-prod` bucket — it has no access to the backup bucket, deliberately (see the `R2_BACKUP_ACCESS_KEY_ID` row below for why), so rotating this one never affects `R2ExportJob`. |
...
| Grafana alert-email SMTP password | reuses `RESEND_API_KEY` | Resend Dashboard → API Keys | **Added 2026-09-05** — Grafana's alerting email contact point authenticates via Resend's SMTP relay using the same key as the app's transactional email (materialized into a second K8s Secret, `tattooos-grafana-smtp`, since Secrets don't cross namespaces). Rotating `RESEND_API_KEY` per the row below also rotates this — no separate step needed, but redeploy Grafana (`kubectl rollout restart deployment/tattooos-grafana -n monitoring`) too, since it doesn't watch the Secret for changes. |
| Production DB connection string | `PROD_DB_CONNECTION_STRING` | DigitalOcean → `pena-e-arte-prod-db` → reset user password, or rotate via Users & Databases | Contains the DB password inline (MySQL connection-string format) — treat rotation of this the same as a password rotation. **Added 2026-09-05, updated 2026-09-06**: also carries `Maximum Pool Size=50` — the instance's real `max_connections` is 151 (resized 2026-09-06 from a 75-connection tier; see `docs/infra/load-test-baseline-2026-09-03.md`'s third follow-up), shared with staging. Preserve this parameter when rotating the password; don't just replace the whole string with a bare DigitalOcean-provided connection string, which won't have it. |
| Staging DB connection string | `STAGING_DB_CONNECTION_STRING` | Same DigitalOcean cluster, staging's own user/database | Same handling as the production connection string above. **Added 2026-09-05, updated 2026-09-06**: also carries `Maximum Pool Size=25` — deliberately small relative to production, on the same shared 151-connection instance, so staging can never meaningfully compete with production for connections. Preserve this parameter when rotating. |
| Cloudflare API token (DNS-01 solver) | `CLOUDFLARE_API_TOKEN` | Cloudflare Dashboard → API Tokens | Used by `cert-manager`'s `letsencrypt-prod-dns01` ClusterIssuer, lives in the `cert-manager` namespace (not `tattooos`) — see `k8s/base/cluster-issuer.yaml`. Rotating does not require reissuing existing certs. |
...
| Google / Apple sign-in client IDs | `VITE_GOOGLE_CLIENT_ID`, `VITE_APPLE_CLIENT_ID` | Google Cloud Console / Apple Developer | Public client identifiers, not secret, but listed here since they're part of the same `tattooos-api-secrets` bundle and breakage here looks identical to a secret-rotation incident. Not previously listed (drift found 2026-09-05). |
...
| R2 backup bucket name | `R2_BACKUP_BUCKET_NAME` | Cloudflare Dashboard → R2 → bucket name (not a credential itself) | **Added 2026-09-05** alongside `R2ExportJob` — production-only, deliberately unset on staging. Not secret, but the bucket must actually exist before this is set to a real value, or the job just no-ops (harmless, logged as a warning, not an error). |
| R2 backup token | `R2_BACKUP_ACCESS_KEY_ID`, `R2_BACKUP_SECRET_ACCESS_KEY` | Cloudflare Dashboard → R2 → Manage API tokens → token named `pena-e-arte-prod-backup` | **Added 2026-09-05.** Deliberately a SEPARATE Account API token from the primary R2 credentials above, scoped to "Object Read & Write" on ONLY the `pena-e-arte-prod-backup` bucket — not the primary bucket. This is why `R2ExportService` downloads via the primary client and uploads via this one instead of using R2's server-side `CopyObject` (which would need one credential with access to both buckets). Rotate the same way as the primary R2 token; if the new token loses access to the backup bucket, `R2ExportJob` fails loudly (a total-failure run throws deliberately), caught by the Hangfire-failure-rate alert in `docs/infra/alerting-runbook.md` — not silent. |
| MaxMind GeoIP account/license key | `MAXMIND_ACCOUNT_ID`, `MAXMIND_LICENSE_KEY` | MaxMind Account Portal → My License Keys → Generate new key | **Added 2026-09-06** alongside the K8s `.mmdb` population CronJob (production only — see `docs/claude/architecture.md`'s "K8s `.mmdb` population" entry). Only the `tattooos-geoip-update` CronJob's own container reads these (via `GeoIpUpdate__AccountId`/`GeoIpUpdate__LicenseKey` in the same `tattooos-api-secrets` Secret) — the API container itself never reads a MaxMind credential, only the resulting `.mmdb` file path. Rotating is low-risk: generate the new key, update the GitHub secret, no redeploy needed (the CronJob picks up the current Secret value on its next scheduled run), then revoke the old key once one successful run with the new key is confirmed (`kubectl get jobs -n tattooos | grep tattooos-geoip-update` / check the CronJob's most recent Job status). |
