...
| Production DB connection string | `PROD_DB_CONNECTION_STRING` | DigitalOcean → `pena-e-arte-prod-db` → reset user password, or rotate via Users & Databases | Contains the DB password inline (MySQL connection-string format) — treat rotation of this the same as a password rotation. **Added 2026-09-05**: also carries `Maximum Pool Size=25` — the instance's real `max_connections` is only 76 total, shared with staging (confirmed via a live `SHOW VARIABLES` query, see `docs/infra/load-test-baseline-2026-09-03.md`'s second follow-up). Preserve this parameter when rotating the password; don't just replace the whole string with a bare DigitalOcean-provided connection string, which won't have it. |
| Staging DB connection string | `STAGING_DB_CONNECTION_STRING` | Same DigitalOcean cluster, staging's own user/database | Same handling as the production connection string above. **Added 2026-09-05**: also carries `Maximum Pool Size=15` — deliberately small, on the same shared 76-connection instance, so staging can never meaningfully compete with production for connections. Preserve this parameter when rotating. |
