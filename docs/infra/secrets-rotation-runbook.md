...
| MaxMind GeoIP account/license key | `MAXMIND_ACCOUNT_ID`, `MAXMIND_LICENSE_KEY` | MaxMind Account Portal → My License Keys → Generate new key | **Added 2026-09-06** alongside the K8s `.mmdb` population CronJob (production only — see `docs/claude/architecture.md`'s "K8s `.mmdb` population" entry). Only the `pena-e-arte-geoip-update` CronJob's own container reads these (via `GeoIpUpdate__AccountId`/`GeoIpUpdate__LicenseKey` in the same `pena-e-arte-api-secrets` Secret) — the API container itself never reads a MaxMind credential, only the resulting `.mmdb` file path. Rotating is low-risk: generate the new key, update the GitHub secret, no redeploy needed (the CronJob picks up the current Secret value on its next scheduled run), then revoke the old key once one successful run with the new key is confirmed (`kubectl get jobs -n pena-e-arte | grep pena-e-arte-geoip-update` / check the CronJob's most recent Job status). |
