...
function isExpired(code: PromoCodeResponse): boolean {
  return code.expiresAt !== null && new Date(code.expiresAt) < new Date();
}

function isExhausted(code: PromoCodeResponse): boolean {
  return code.maxRedemptions !== null && code.redemptionCount >= code.maxRedemptions;
}

export function PromoCodeCard({ promoCode }: PromoCodeCardProps) {
        ...
        <CardContent className="p-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className={cn(
              ...
              {expired ? "Expired" : exhausted ? "Exhausted" : promoCode.isActive ? "Active" : "Inactive"}
            </span>
