 ...
 * treatment — a row is always either actionable or explained, never a dead-end button. The artist
 * connects and verifies their own accounts (OAuth consent belongs to the account holder).
 */
export function ArtistSocialTab({ artistId, firstName, slug, canManage, isOwnProfile }: ArtistSocialTabProps) {
  ...
  // Who may connect / verify / edit / disconnect: an owner (any artist in the studio) or the artist
  // on their own profile. Matches the backend — ArtistAndAbove policies plus a handler-side
  // ownership guard — so a button shown here never 403s. Broader than `canManage` (owner-only),
  // which still governs studio-management actions elsewhere on the page.
  ...
  // The read-only branch is now only reachable by an artist looking at a colleague's profile.
  const helper = canManageSocial
    ? isOwnProfile
